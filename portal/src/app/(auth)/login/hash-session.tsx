...
import { destinationFor } from "@/lib/auth-links";
...
export function HashSession({ url, anonKey, next }: { url: string; anonKey: string; next: string }) {
  ...
  useEffect(() => {
    ...
    const finish = async (access_token: string, refresh_token: string, type: string | null) => {
      setState("working");
      const supabase = createBrowserClient(url, anonKey);
      const { error } = await supabase.auth.setSession({ access_token, refresh_token });
      if (error) return fail("That sign-in link couldn't be verified. Request a new one below.");
      window.location.replace(destinationFor(type, next));
    };
    ...
    const id = window.setTimeout(() => {
      ...
      else if (access_token && refresh_token) void finish(access_token, refresh_token, type);
    }, 0);
